...
def make_plots_var3(plot_name, num_spectra, fracsctr, fracsctr_err, gamma, \
                    gamma_err, bb_t, bb_t_err, fs_sinefit, bbt_sinefit):
    """
    ...
    bbt_max = tinybins[np.argmax(bbt_sinefit)]
    ...
    ax2.plot(tinybins, bbt_sinefit, c='black', lw=2)
    ax2.errorbar(phase, bb_t, yerr=bb_t_err, lw=0, ecolor='green', \
                 marker='.', ms=10, mec='green', mfc='green', elinewidth=2,
                 capsize=2)
    ax2.vlines(bbt_max, 0.798, 0.810, lw=1, linestyles='dashed')
    # ax2.vlines(bbt_max - 4.3787e-03, 0.798, 0.810, lw=1, linestyles='dotted')
    # ax2.vlines(bbt_max + 4.3787e-03, 0.798, 0.810, lw=1, linestyles='dotted')
    ax2.tick_params(axis='x', labelsize=18, bottom=True, top=True, \
                    labelbottom=False, labeltop=False)
    ax2.tick_params(axis='y', labelsize=18, left=True, right=True, \
                    labelleft=True, labelright=False)
    ax2.set_ylabel(r'diskbb: T$_{in}$ (keV)', fontproperties=font_prop)
...
def main(log_file):
    ...
    bb_norm_err, line_e, line_e_err, sigma, sigma_err, e_norm, e_norm_err, \
    num_spectra = read_log_file(log_file)
    ...
    if gamma[0] == gamma[1] and gamma_err[1].is_integer():
        gamma_err = np.repeat(gamma_err[0], num_spectra)
        var_gamma = False

    if fracsctr[0] == fracsctr[1] and fracsctr_err[1].is_integer():
        fracsctr_err = np.repeat(fracsctr_err[0], num_spectra)
        var_fracsctr = False

    if bb_t[0] == bb_t[1] and bb_t[1].is_integer():
        bb_t_err = np.repeat(bb_t_err[0], num_spectra)
        var_bb_t = False

    # if bb_norm[0] == bb_norm[1] and bb_norm_err[1].is_integer():
    #     bb_norm_err = np.repeat(bb_norm_err[0], num_spectra)
    #     var_bb_norm = False

    if line_e[0] == line_e[1] and line_e_err[1].is_integer():
        line_e_err = np.repeat(line_e_err[0], num_spectra)
        var_line_e = False

    if sigma[0] == sigma[1] and sigma_err[1].is_integer():
        sigma_err = np.repeat(sigma_err[0], num_spectra)
        var_sigma = False

    if e_norm[0] == e_norm[1] and e_norm_err[1].is_integer():
    ...
    print "simpl fracsctr phase: %.4e +- %.4e" % (fracsctr_phase, fracsctr_phase_err)

    phase_diff = fracsctr_phase - bb_t_phase
    if phase_diff < 0:
        phase_diff += 1.0
    elif phase_diff > 1.0:
        phase_diff -= 1.0
